...
graph = Graph(user="neo4j",host="localhost")
    ...
    if instances['Reservations'] == []:
        ...
        for instance in instances['Reservations']:
            ...
            if not instance['Instances'][0]['State']['Code'] == 48:
                ...
                graphSubnet = graph.find(label="Subnet",property_key='subnetId',property_value=subnetId)
...
def create_elb():
    ...
    for elb in elbs:
        ...
        for subnet in elb['Subnets']:
            tx = graph.begin()
            graphSubnet = graph.find(label="Subnet",property_key='subnetId',property_value=subnet)
        ...
        for instance in elb["Instances"]:
            try:
                tx = graph.begin()
                graphInstance = graph.find(label="EC2",property_key='instanceId',property_value=instance['InstanceId'])
...
def create_relationships():
    try:
        ...
        for sg in securityGroups['SecurityGroups']:
            graphSg = graph.find(label="SecurityGroup",property_key='securityGroupId',property_value=sg['GroupId'])
            ingressRules = sg['IpPermissions']
            for rule in ingressRules:
                if (rule['UserIdGroupPairs'] != []):
                    for group in rule['UserIdGroupPairs']:
                        tx = graph.begin()
                        graphFromSg = graph.find(label="SecurityGroup",property_key='securityGroupId',property_value=group['GroupId'])
                ...
                if (rule['IpRanges'] != []):
                    for cidr in rule['IpRanges']:
                        tx = graph.begin()
                        try:
                            graphCidr = graph.find(label="IP",property_key='cidr',property_value=cidr['CidrIp'])
                        except:
            ...
            if instances['Reservations'] == []:
                ...
                for instance in instances['Reservations']:
                    tx = graph.begin()
                    instanceId = instance['Instances'][0]['InstanceId']
                    graphEc2 = graph.find(label="EC2",property_key='instanceId',property_value=instanceId)
            ...
            for db in databases:
                ...
                for dbSg in dbSgs:
                    if (dbSg['VpcSecurityGroupId'] == sg['GroupId']):
                        tx = graph.begin()
                        graphRds = graph.find(label="RDS",property_key='rdsId',property_value=db['DBInstanceIdentifier'])
            ...
            for elb in elbs:
                ...
                for elbSg in elbSgs:
                    if (elbSg == sg['GroupId']):
                        tx = graph.begin()
                        graphElb = graph.find(label="ELB",property_key='name',property_value=elb['LoadBalancerName'])
